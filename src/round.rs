...
fn apply_harness_label(repo: &Repo, id: &str, json: &Value, model: &mut String) {
    ...
    let brief_files = if repo.research_aider && !repo.gate.is_empty() {
        if let Some(research) = json.get("research") {
            if let Some(brief) = research.as_str() {
                let (existing, missing) = brief_files(brief, &repo.repo);
                // Gate condition: have files (existing is not empty) and missing is empty
...
pub fn harness_for(labels: &[&str], model: &str, brief_files: Option<&[String]>) -> (String, Option<String>) {
